...
@export var launch_pad_impulse: float = 700.0 # px/s added along the floor normal, same axis/style as jump_impulse, the instant you enter a launch pad zone while grounded - an automatic pop that needs no button, terrain's counterpart to the manual jump. Existing horizontal momentum carries through unchanged just like a real jump; the very next physics frame's on_floor -> airborne transition triggers the existing launch-quality system exactly like any other liftoff. Raised from 480 (was barely bigger than jump_impulse's 520 despite being the one automatic, guaranteed-for-everyone launch on the course) after playtest feedback that it felt too weak to read as a real event
